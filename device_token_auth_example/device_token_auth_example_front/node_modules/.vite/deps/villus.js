...
function hash(x) {
  let h2, i, l;
  for (h2 = 5381 | 0, i = 0, l = x.length | 0; i < l; i++) {
    h2 = (h2 << 5) + h2 + x.charCodeAt(i);
  }
  return h2 >>> 0;
}
...
function resolveClient() {
  var _a;
  const vm = getCurrentInstance();
  let client = vm && inject(VILLUS_CLIENT, (_a = vm === null || vm === void 0 ? void 0 : vm.provides) === null || _a === void 0 ? void 0 : _a[VILLUS_CLIENT]);
  if (client)
    setActiveClient(client);
...
function useQuery(opts) {
  ...
  if (isRef(query)) {
    watch(query, () => execute2());
  }
  let stopVarsWatcher;
  const isWatchingVariables = ref(false);
  ...
  beginWatchingVars();
  const api = { data, isFetching, isDone, error, execute: execute2, unwatchVariables, watchVariables, isWatchingVariables };
  const vm = getCurrentInstance();
  if (fetchOnMount) {
...
var QueryImpl = defineComponent({
  ...
  setup(props, ctx) {
    function createRenderFn(api) {
      const { data, error, isFetching, isDone, execute: execute2, watchVariables, isWatchingVariables, unwatchVariables } = api;
...
var MutationImpl = defineComponent({
  name: "Mutation",
  props: {
    query: {
      type: [String, Object],
  ...
  setup(props, ctx) {
    const { data, isFetching, isDone, error, execute: execute2 } = useMutation(props.query);
    return () => {
      return normalizeChildren(ctx, {
        data: data.value,
        isFetching: isFetching.value,
...
var SubscriptionImpl = defineComponent({
  name: "Subscription",
  props: {
    query: {
      type: [String, Object],
...
var Subscription = SubscriptionImpl;
